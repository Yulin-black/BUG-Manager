...
class CreateDirForm(AddCssCodeFrom, forms.ModelForm):
    ...
    def __init__(self,parent=None,request=None,*args,**kwargs):
    ...
    def clean_name(self):
        ...
        if self.parent:
            exists = models.CosFileDir.objects.filter(
                project= self.request.user.project ,parent_id=self.parent, name=name, file_type=1
            ).exists()
        else:
            exists = models.CosFileDir.objects.filter(
                project=self.request.user.project, parent__isnull=True, name=name, file_type=1
            ).exists()
        if exists:
