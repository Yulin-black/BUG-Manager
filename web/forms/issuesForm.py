...
class IssuesModelForm(AddCssCodeFrom, forms.ModelForm):
    class Meta:
        model = models.Issues
        fields = ['issues_type', 'subject', "module", "desc", "state", "priority", "assign", "attention", "mode",
    ...
    def __init__(self, request=None, *args, **kwargs):
        ...
        if request:
            ...
            # 关注者 和 参与者
            total_user = list(
                models.ProjectUser.objects.filter(project=self.request.user.project).values_list("invitee__id",
                                                                                                 "invitee__username"))
            self.fields['attention'].choices = [(self.request.user.user.id,
                                                 self.request.user.user.username)] + total_user
            self.fields['assign'].choices = [("", "没有选中任何选项")] + total_user

            # 父问题
            parent_list = list(
                models.Issues.objects.filter(project=self.request.user.project).values_list("id", "subject"))
            self.fields['parent'].choices = [("", "没有选中任何选项")] + parent_list

            # 问题类型 和 模块
            self.fields['issues_type'].choices = models.IssuesType.objects.filter(
                project=self.request.user.project).values_list("id", "title")
            self.fields['module'].choices = [("", "没有选中任何选项")] + list(
                models.Module.objects.filter(project=self.request.user.project).values_list("id", "title"))


class InviteModelForm(AddCssCodeFrom, forms.ModelForm):
    class Meta:
